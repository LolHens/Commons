...
import org.lolhens.commons.swing.renderer.primitives.Line;
                ...
                return builder.build().transform((oldX, oldY, oldWidth, oldHeight) -> {
                    ...
                    for (ChartVal value : getValues()) {
                        ...
                        builder.append(new CenteredText(value.getName(), 0, !autoMirror ? 2 : 0).transform(x, getY0(), 0, 0).transform((x1, y1, width1, height1) ->
                                new org.lolhens.commons.swing.renderer.Rectangle(x1, y1 + (fontMetrics.get().getHeight() * (autoMirror ? y - getY0() <= 0 ? 1 : -1 : 1) * (scale - 0.5)) * Math2.abs1(oldHeight) - 4, width1, height1)));
                    }
