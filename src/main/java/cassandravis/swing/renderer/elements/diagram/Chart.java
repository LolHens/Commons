...
import cassandravis.util.collection.Collections2;
import cassandravis.util.hash.HashBuilder;
import cassandravis.util.schema.ColorSchema;
        ...
        public class GraphCaption implements Renderer {
            ...
            public Renderer render(Renderer renderer) {
                ...
                return getFontMetrics.with(builder.build().transform((x1, y1, width, height) ->
                        new Rectangle(x1, y1, width, getFontMetrics.get().getHeight() * (values.size()))));
            }
        }
    }
}
